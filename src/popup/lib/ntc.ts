  ...
  name: function (color: string) {
    ...
    for (var i = 0; i < ntc.names.length; i++) {
      ...
      ndf1 =
        ...
        Math.pow(g - Number(ntc.names[i][3]), 2) +
        Math.pow(b - Number(ntc.names[i][4]), 2);
      ndf2 =
        Math.pow(h - Number(ntc.names[i][5]), 2) +
        Math.pow(s - Number(ntc.names[i][6]), 2) +
        Math.pow(l - Number(ntc.names[i][7]), 2);
      ndf = ndf1 + ndf2 * 2;
      if (df < 0 || df > ndf) {
        df = ndf;
        cl = i;
    ...
    return cl < 0 ? "Unknown" : ntc.names[cl][1];
  },
